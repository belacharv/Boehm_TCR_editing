...
def fit_model_to_data(cd4_cd4_obs, cd8_cd8_obs, total_sequences=386):
    ...
    valid_solutions = []
    for guess in [(0.9,386),(0,0)]:#(0.8, 300), (0.2, 100), (0.1, 50), (0.9, 350)]:
        try:
            ...
            if (0 <= solution[0] <= 1 and 0 <= solution[1] <= total_sequences):
                # Check if this is a new solution (avoid duplicates)
                is_new = True
                for existing_sol in valid_solutions:
                    if (abs(solution[0] - existing_sol[0]) < 1e-6 and 
                        abs(solution[1] - existing_sol[1]) < 1e-6):
                        is_new = False
                        break
                
                if is_new:
                    valid_solutions.append((solution[0], solution[1]))
        except:
            continue
    
    # Return the first valid solution (or modify to return all solutions)
    if valid_solutions:
        print(f"Found {len(valid_solutions)} valid solution(s):")
        for i, sol in enumerate(valid_solutions):
            print(f"  Solution {i+1}: Pc = {sol[0]:.6f}, n4 = {sol[1]:.6f}")
        print(valid_solutions)
        return valid_solutions[0], valid_solutions[1]
...
for i in range(n_simulations):
    ...
    first_solution, second_solution = fit_model_to_data(cd4_cd4, cd8_cd8)
    fitted_Pc, fitted_n4 = first_solution
    fitted_Pc2, fitted_n8 = second_solution
    ...
    if fitted_Pc is not None:
        recovery_results.append({
            ...
            'error_n8': abs(fitted_n8 - true_n8)
        })
...
#df_recovery.to_csv("250908_parameter_recovery_pc_estimation.csv", index=False)
